...
require_once($CFG->dirroot.'/admin/user/user_bulk_sms_lib.php');

require_once($CFG->dirroot.'/phlcohort/user/upload/phlusercsv.php');
...
function switch_date_month_lol($DateTime){
...
function whoever_gives_a_fuck($userid)
{
    global $DB;
    $sql = "
select c.online,c.name,u.firstname+' '+u.lastname as hoten,
co.fullname,e.id as enrolid,cxt.id as contextid,r.id as roleid,ue.id as user_enrol_id,co.id as courseid
from mdl232x0_cohort c,
mdl232x0_cohort_members cm,
mdl232x0_user u,
mdl232x0_user_enrolments ue,
mdl232x0_enrol e,
mdl232x0_course co,
mdl232x0_context cxt,
mdl232x0_role_assignments r
where c.id = cm.cohortid
and cm.userid = u.id
and u.id = ue.userid
and ue.enrolid = e.id
and e.courseid = co.id
and co.id = cxt.instanceid
and cxt.id = r.contextid
and r.userid = u.id
and c.courseid = co.id
and u.id = $userid
";
    $check_if_enrol = $DB->get_records_sql($sql,array());
    foreach($check_if_enrol as $a)
    {
        $a1 = $a->online;
        $a2 = $a->name;
        $a3 = $a->hoten;
...
$returnurl = new moodle_url('/phlcohort/user/upload/index.php');
...
if (empty($iid)) {
    ...
    if ($formdata = $mform1->get_data()) {
        ...
        $phlCSV = new phl_user_csv($iid, 'uploaduser');
        $cir = new csv_import_reader($iid, 'uploaduser');
        
        $userfile = $formdata->userfile;
        $file = $mform1->get_user_file($userfile);
        
        $content = $mform1->get_excel_file_content($file);
        
        $readcount = $phlCSV->load_csv_content($content, $formdata->encoding, $formdata->delimiter_name);
        $csvloaderror = $phlCSV->get_error();
        unset($content);
        
        $filecolumns=$phlCSV->get_columns();
        ...
        $search .= html_writer::link("../../csv/Form_Upload_Student.xlsx","Táº£i file máº«u",array('class' =>''));
        $search .= html_writer::end_div();
        $search .= html_writer::end_div();
        echo $search;
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    $cir->init();
    $linenum = 2; //column header is first line
    ...
    while ($line = $cir->next()) {
        ...
        if ($optype == UU_USER_ADDNEW or $optype == UU_USER_ADDINC) {
            ...
            //heavily customized
            $check_username = $user->profile_field_cmtnd;
            //custom
            $cohortid = $DB->get_record('cohort', array('idnumber' =>$cohort1));
            $is_online = $cohortid->online;
            if($cohortid->idnumber == '')
            {
                $upt->track('status', "Lá»›p $cohort1 khÃ´ng tá»“n táº¡i", 'error');
                $upt->track('cohort1', $errorstr, 'error');
                $error = true;
            }
            else{
                ...
                {
                    $upt->track('status', "KhÃ³a há»c $course3 khÃ´ng tá»“n táº¡i", 'error');
                    $upt->track('course3', $errorstr, 'error');
                    $error = true;
                }
                else
                {
                    ...
                    {
                        $upt->track('status', "KhÃ³a há»c $course3 khÃ´ng khá»›p vá»›i lá»›p $cohort1 - Online : $is_online", 'error');
                        $upt->track('cohort1', $errorstr, 'error');
                        $error = true;
                    }
                }
        ...
        if (empty($user->username)) {
            $upt->track('status', 'It seems you have forgot your username', 'error');
        ...
        if ($existinguser) {
            ...
            $spot2 = whoever_gives_a_fuck($user->id);
            ...
            // custom C
            if(!is_null($course3) && $is_online ==1)
            {
                $get_course_id = $DB->get_record('course',array('idnumber'=>$course3));
                
                $sql_get_enrol = "select top 1 id from mdl232x0_enrol where courseid = $get_course_id->id order by id desc";
                $enrol_arrayhh = $DB->get_records_sql($sql_get_enrol,array());
                foreach($enrol_arrayhh as $hmm_enrolh)
                {
                    $latest_enrol = $hmm_enrolh->id;
                }
                $get_enrol = $DB->get_record('enrol',array('courseid'=>$get_course_id->id));
                $get_context_id = $DB->get_record('context',array('instanceid'=>$get_course_id->id));
                $is_role_exist = $DB->get_record('role_assignments',array('roleid'=>5,'contextid'=>$get_context_id->id,'userid'=>$user->id));
                if(is_null($is_role_exist->id))
                {
                    $role->roleid=5;
                    $role->contextid=$get_context_id->id;
                    $role->userid=$user->id;
                    $role->component='enrol_cohort';
                    $role->itemid=$latest_enrol;
                    $role->timemodified=strtotime(switch_date_month_lol(date("d/m/Y")));
                    $role->modifierid=2;
                    $role->sortorder = 0;
        ...
        $spot3 = whoever_gives_a_fuck($user->id);
        ...
        foreach ($filecolumns as $column) {
            ...
            if (!empty($user->$column)) {
                ...
                if (!isset($cohorts[$addcohort])) {
                    ...
                    $spot7 = whoever_gives_a_fuck($user->id);
                }
                ...
                if (is_object($cohorts[$addcohort])) {
                    ...
                    if (!$DB->record_exists('cohort_members', array('cohortid'=>$cohort->id, 'userid'=>$user->id))) {
                        $spot9 = whoever_gives_a_fuck($user->id);
                        $imposter1 = $cohort->id.' | '. $user->id;
                        $what_u_got = cohort_add_member($cohort->id, $user->id);
                        ...
                        $spot8 = whoever_gives_a_fuck($user->id);
                    }
                } else {
                    // error message
            ...
            $spot5 = whoever_gives_a_fuck($user->id);
        }
        ...
        foreach ($filecolumns as $column) {
            ...
            //if($is_online != 1 && !empty($course3)) // since its add new, course3 will not be empty
            //{
            //    $shortname = null;
            //}
            //$shortname = 'UL2021';
            if (!array_key_exists($shortname, $ccache)) {
                ...
                $acr = $DB->get_record('cohort',array('idnumber'=>$addcohort));
                if($acr->online != 1)
                {
                    $shortname = null;   
                }
                if (!$course = $DB->get_record('course', array('idnumber'=>$shortname), 'id, idnumber')) 
                {
                    if(!empty($course3))
                    {
                        $upt->track('enrolments', "ThÃªm má»›i há»c viÃªn vÃ o lá»›p $addcohort - OFF", 'error');
                        continue;
                    }
                    else 
                    {
                        $upt->track('enrolments', "Cáº­p nháº­t há»c viÃªn vÃ o lá»›p $addcohort - OFF", 'error');
                        continue;
                    }
                }
                else 
                {
                    if(!empty($course3))
                    {
                        $upt->track('enrolments', "ThÃªm má»›i há»c viÃªn vÃ o lá»›p $addcohort khÃ³a $shortname", 'error');
                        continue;
                    }
                    else
                    {
                        $upt->track('enrolments', "Cáº­p nháº­t há»c viÃªn vÃ o lá»›p $addcohort khÃ³a $shortname", 'error');
                        continue;
                    }
                }
            ...
            $spot6 = whoever_gives_a_fuck($user->id);
        }
        $spot4 = whoever_gives_a_fuck($user->id);
        // add to cohort first, it might trigger enrolments indirectly - do NOT create cohorts here!
        if($is_online==1){
            $enrol_exist = $DB->get_record('user_enrolments',array('userid'=>$user->id));
            //$enrol_exist_l = $DB->get_record('enrol',array('courseid'=>$get_course_id->id));
            
            if(is_null($enrol_exist->id))
            {
                $custom_enrol->status = 0;
                $custom_enrol->enrolid = $latest_enrol;
                $custom_enrol->userid = $user->id;
                $custom_enrol->timestart = 0;
                $custom_enrol->timeend = 0;
                $custom_enrol->modifierid = 240;
                $custom_enrol->timecreated = time();
                $custom_enrol->timemodified = time();
                $custom_enrol->id = $DB->insert_record('user_enrolments',$custom_enrol);
                $REVIEW_ue = $DB->get_record('user_enrolments',array('id'=>$custom_enrol->id));
            }
        }
        // find course enrolments, groups, roles/types and enrol periods
        // for update reason
    ...
    $cir->close();
    $cir->cleanup(true);
    $spot1 = whoever_gives_a_fuck($user->id);
    echo $OUTPUT->box_start('boxwidthnarrow boxaligncenter generalbox', 'uploadresults');
    echo "<p>addcohort returns $cohort->idnumber</p>";
    echo "<p>1) $spot1</p>";
    echo "<p>2) $spot2</p>";
    echo "<p>3) $spot3</p>";
    echo "<p>4) $spot4</p>";
    ...
    if ($optype != UU_USER_UPDATE) {
        echo get_string('userscreated', 'tool_uploaduser').': '.$usersnew.'<br />'; // thanh vien duoc tao moi
    }
    if ($optype == UU_USER_UPDATE or $optype == UU_USER_ADD_UPDATE) {
        echo get_string('usersupdated', 'tool_uploaduser').': '.$usersupdated.'<br />'; // thanh vien duoc cap nhat
    }
    if ($allowdeletes) {
    ...
    echo get_string('usersweakpassword', 'tool_uploaduser').': '.$weakpasswords.'<br />'; // weak password
    echo get_string('errors', 'tool_uploaduser').': '.$userserrors.'</p>'; // loi
    echo $OUTPUT->box_end();
