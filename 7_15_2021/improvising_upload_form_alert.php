        ...
        while ($row = $cir->next()) {
            ...
            if (!empty($hash['idnumber'])) {
                if (isset($idnumbers[$hash['idnumber']]) || $DB->record_exists('cohort', array('idnumber' => $hash['idnumber']))) {
                    $cohorts[$rownum]['warnings'][] = "MÃ£ lá»›p Ä‘Ã£ tá»“n táº¡i";
            ...
            if (empty($hash['name'])) {
                $cohorts[$rownum]['errors'][] = "MÃ£ lá»›p Ä‘ang Ä‘á»ƒ trá»‘ng<br>";
            }
            if (empty($hash['idnumber'])) {
                $cohorts[$rownum]['errors'][] ="ChÆ°a cÃ³ mÃ£ lá»›p<br>";
            }
            //2
            $chuongtrinh = $hash['cohorttype'];
            $array_sql = "
        select id from mdl232x0_course where idnumber = N'$chuongtrinh'
";
            $array = $DB->get_records_sql($array_sql,array());
            foreach($array as $a){
                $check_course= $a->id;
            }
            if (empty($check_course)) {
                $cohorts[$rownum]['errors'][] ="Nháº­p sai chÆ°Æ¡ng trÃ¬nh: $chuongtrinh<br>";
            }
            //custom
            if($hash['online']==0 || empty($hash['online'])){
                $city1 = $hash['city_test'];
                $array_sql1 = "
        select id from mdl232x0_cohortphl_khuvuc where tenkhuvuc = N'$city1'
";
                $array1 = $DB->get_records_sql($array_sql1,array());
                foreach($array1 as $a1){
                    $check_city1= $a1->id;
                }
                if (empty($check_city1)) {
                    $cohorts[$rownum]['errors'][] ="Nháº­p sai thÃ nh phá»‘ thi: $city1<br>";
                }
                
                //3
                $city2 = $hash['city_training'];
                $array_sql2 = "
        select id as lol from mdl232x0_cohortphl_khuvuc where tenkhuvuc = N'$city2'
";
                $array2 = $DB->get_records_sql($array_sql2,array());
                foreach($array2 as $a2){
                    $check_city2= $a2->lol;
                }
                if (empty($check_city2)) {
                    $cohorts[$rownum]['errors'][] ="Nháº­p sai thÃ nh phá»‘ huáº¥n luyá»‡n: $city2<br>";
                }
...
";
        echo "<table>
  <tr>
    <th>Sá»‘ lÆ°á»£ng cohort Ä‘Æ°á»£c cáº­p nháº­t</th>
    <th>Sá»‘ lÆ°á»£ng cohort Ä‘Æ°á»£c thÃªm má»›i</th>
  </tr>
  <tr>
