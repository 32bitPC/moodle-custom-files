...
function cohort_upload_add_cohort_debut($cohort) {
    ...
    if(is_null($cohort_id))
    {
        ...
        if(isset($cohort->online) && $cohort->online==1)
        {
            $cohort->address_training = '';
            $cohort->address_test = '';
            $city_training->id = 0;
            $city_test->id = 0;
        }
        ...
        $sql_update = "
    update mdl232x0_cohort
    set
    cohorttype = N'$cohort->cohorttype',
    area = N'$cohort->area',
    aduser = N'$cohort->aduser',
    trainer = N'$cohort->trainer',
    datetest = $datetest,
    note = N'$cohort->note',
    office = N'$cohort->office',
    testcode = N'$cohort->testcode',
    examiner = N'$cohort->examiner',
    datestart = $datestart,
    dateend = $dateend,
    address_training = N'$cohort->address_training',
    city_training = $city_training->id,
    pss_quantity = $cohort->pss_quantity,
    address_test = N'$cohort->address_test',
    cancel_flag = $cohort->cancel_flag,
    error_desc = N'$cohort->error_desc',
    city_test = $city_test->id,
    test_form = N'$cohort->test_form',
    ...
    if($khoahoc==0)
        ...
        if($khuvuc==0 && $khoahoc==0)
            ...
            if(($mien>0 && $khuvuc>0 && $quanhuyen>0 && $khoahoc>0) || $cohort->online=="Yes")
            {
                ...
                $cohort->description = $cohort->trainer;
                if (!isset($cohort->descriptionformat)) {
                    $cohort->descriptionformat = FORMAT_HTML;
                }
                if (!isset($cohort->visible)) {
