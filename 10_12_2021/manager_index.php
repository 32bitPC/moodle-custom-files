...
require('../../config.php');
require($CFG->dirroot.'/phlcohort/manager/lib.php');
require_once($CFG->libdir.'/adminlib.php');

$contextid = optional_param('contextid', 0, PARAM_INT);
$page = optional_param('page', 0, PARAM_INT);
$searchquery  = optional_param('search', '', PARAM_RAW);
//$related_searchquery = optional_param('related_search', '', PARAM_RAW);
$showall = optional_param('showall', false, PARAM_BOOL);

require_login();
require_once($CFG->libdir . '/accesslib.php');
//custom_limited_access($USER->id);
if ($contextid) {
...
if ($category) {
    ...
    $PAGE->set_url('/phlcohort/manager/index.php', array('contextid'=>$context->id));
...
if(!empty($related_searchquery))
{
    $cohorts_custom = cohort_get_all_cohorts_online($page, 25, $related_searchquery);
    
}
else {
    if ($showall) {
        //  $cohorts = cohort_get_all_cohorts($page, 25, $searchquery);
        $cohorts = cohort_get_all_cohorts_online($page, 25, $searchquery);
...
///
if ($cohorts_custom['allcohorts'] > 0) {
    if ($related_searchquery === '') {
        $count = ' ('.$cohorts_custom['allcohorts'].')';
    } else {
        $count = ' ('.$cohorts_custom['totalcohorts'].'/'.$cohorts_custom['allcohorts'].')';
    }
}
///
...
///
if ($related_searchquery) {
    $params['related_search'] = $related_searchquery;
}
///
if ($showall) {
    $params['showall'] = true;
}
$baseurl = new moodle_url('/phlcohort/manager/index.php', $params);

if ($editcontrols = cohort_edit_controls_manager($context, $baseurl)) {
...
$search .= html_writer::label(get_string('searchcohort', 'cohort'), 'cohort_search_q', true,
    ...
    echo $search;
    echo "
    <form action='/report/report/completion/quiz.php?'>
  <input type='hidden' id='fname' name='cohortid' value='$searchquery'>
  <label for='course_fullname'>Chọn khóa học:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>&nbsp;
  <select style='width: 195px; height: 40px;' name='quizid' id='course_fullname'>
    <option value='234'>Phú Hưng Trí Việt</option>
    <option value='235'>Phú Hưng Phát Lộc</option>
    <option value='47mof'>Khởi nghiệp thành công - PSS</option>
    <option value='322'>Phú Hưng Thịnh Vượng (UL)</option>
    <option value='324'>Phú Hưng Đại Phát</option>
    <option value='334'>Pháp Chế Đại lý – Agency Compliance</option>
  </select>
<input type='hidden' id='fname' name='format' value='excelcsv'>
  &nbsp;&nbsp;&nbsp;<input type='submit' value='Xuất báo cáo'>
</form>
";
    echo "<br>";
    //echo " related_searchquery : ".$related_searchquery;
    // Output pagination bar.
    echo $OUTPUT->paging_bar($cohorts['totalcohorts'], $page, 25, $baseurl);
    ///
    echo $OUTPUT->paging_bar($cohorts_custom['totalcohorts'], $page, 25, $baseurl);
    ///
    $data = array();
    $editcolumnisempty = true;
    ///
    foreach($cohorts_custom['cohorts'] as $cohort) {
        $line = array();
        ...
        if ($showall) {
            if ($cohortcontext->contextlevel == CONTEXT_COURSECAT) {
                $line[] = html_writer::link(new moodle_url('/phlcohort/manager/index.php' ,
        ...
        if (empty($cohort->component)) {
            ...
            $showhideurl = new moodle_url('/phlcohort/manager/edit.php', $urlparams + array('sesskey' => sesskey()));
    ...
    foreach($cohorts['cohorts'] as $cohort) {
        ...
        if (empty($cohort->component)) {
            ...
            if ($cohortmanager) {
                ...
                $buttons[] = html_writer::link(new moodle_url('/phlcohort/manager/edit.php', $urlparams + array('delete' => 1)),
                    $OUTPUT->pix_icon('t/delete', get_string('delete')),
                    array('title' => get_string('delete')));
                $buttons[] = html_writer::link(new moodle_url('/phlcohort/manager/edit.php', $urlparams),
                    $OUTPUT->pix_icon('t/edit', get_string('edit')),
                    array('title' => get_string('edit')));
                $editcolumnisempty = false;
            }
            if ($cohortcanassign) {
                $buttons[] = html_writer::link(new moodle_url('/phlcohort/manager/assign.php', $urlparams),
                    $OUTPUT->pix_icon('i/users', get_string('assign', 'core_cohort')),
                    array('title' => get_string('assign', 'core_cohort')));
                //download button
                
                //$urlparams = array('cohortid' => $cohort->id, 'course' => $CFG->year_course['2019']);
                //$buttons[] = html_writer::link(new moodle_url('/phlcohort/report/completion/index.php', $urlparams),
                //	$OUTPUT->pix_icon('i/user', "Kết quả học 2019"),
                //array('title' => "Kết quả học 2019"));
                //$urlparams2019 = array('cohortid' => $cohort->id, 'course' => $CFG->year_course['2019'],'format'=>'excelcsv');
                //$buttons[] = '<a title="Download" href="'.new moodle_url('/phlcohort/report/completion/index.php', $urlparams).'"><i class="icon fa fa-download fa-fw " aria-hidden="true" title="Download Kết quả học 2019" aria-label="Download Kết quả học 2019"></i></a>';
                
                
                //$urlparams = array('cohortid' => $cohort->id, 'course' => $CFG->year_course['2020']);
                //$buttons[] = html_writer::link(new moodle_url('/phlcohort/report/completion/index.php', $urlparams),
                //	$OUTPUT->pix_icon('i/user', "Kết quả học 2020"),
                //array('title' => "Kết quả học 2020"));
                //$urlparams2020 = array('cohortid' => $cohort->id, 'course' => $CFG->year_course['2020'],'format'=>'excelcsv');
                //$urlparamsul = array('cohortid' => $cohort->id, 'quizid' => 322,'format'=>'excelcsv');
                $buttons[] = '<a id="adownload'.$cohort->id.'" style="display:none;" title="Download" href=""><i class="icon fa fa-download fa-fw " aria-hidden="true" title="Download Kết quả học 2020" aria-label="Download Kết quả học 2020"></i></a>';
                $buttons[]=GetCourseByCohortID($cohort->id);
