...
echo "STD_FIELDS = ".$STD_FIELDS."<br>";
$PRF_FIELDS = array();

//Test
//$DB->get_records_sql('update {user_info_field} set name=\'AD\' where id=16');
//$temp=$DB->get_records_sql('select name from {user_info_field} where id=16');
//var_dump($temp);
//End code
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    while ($line = $cir->next()) {
        ...
        if ($optype == UU_USER_ADDNEW or $optype == UU_USER_ADDINC) {
            ...
            $error = false;
            //if ($user->course3 == 'PSS') {
            //    $upt->track('status', 'What the fuck is this course', 'error');
            //    $upt->track('coursse3', $errorstr, 'error');
            //    $error = true;
            //}
            // see this , stop deleting
        ...
        foreach ($filecolumns as $column) {
            ...
            $shortname = $user->{'course3'};                // <----- Jackpot
            //if($is_online!=1 && !(is_null($cohort1)) && !(is_null($course3)))
            if($is_online!=1 && !(is_null($cohort1)) && !(is_null($course3)))
            {
                $shortname = 'KhÃ´ng enrol vÃ o khÃ³a há»c do lá»›p offline';
            }
            //if(is_null($is_online) && is_null($cohort1) && is_null($course3))
            //{
            //    $shortname = 'Cáº­p nháº­t thÃ nh cÃ´ng';
            //}
        ...
        {
            $sql_get_correct_enrol = "select top 1 id from mdl232x0_enrol where courseid = $course_idnumber->id order by id desc";
            $enrol_array = $DB->get_records_sql($sql_get_correct_enrol,array());
            foreach($enrol_array as $hmm_enrol)
            {
                $latest_enrol_id = $hmm_enrol->id;
            }
            $custom_enrol->status = 0;
            $custom_enrol->enrolid = $latest_enrol_id;
