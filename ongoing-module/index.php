        ...
        {  
            ...
            for($i=0;$i<$max;$i++){
                ...
                if($quiz_result->num_rows>0){
                    ...
                    $deg1 = '266.3deg';
                    echo "total grade is ".$total_grades."<br>";
                    $total_grades_percent = $total_grades;
                    
                    $in_deg = $total_grades*3.6;
                    $myfile = fopen("in_deg.txt", "w") or die("Unable to open file!");
                    fwrite($myfile, $in_deg."deg");
                    fclose($myfile);
                    $myfile = fopen("in_deg.txt", "r") or die("Unable to open file!");
                    echo "deg send =  ".file_get_contents('./in_deg.txt', true)."<br>";
                    $send_deg = file_get_contents('./in_deg.txt', true);
                    fclose($myfile);
                    echo "deg send 2 =  ".$send_deg."<br>";
                    
                    
                    $myfile = fopen("deg1.txt", "w") or die("Unable to open file!");
                    fwrite($myfile, $deg1);
                    fclose($myfile);
                    
                    $myfile = fopen("deg1.txt", "r") or die("Unable to open file!");
                    echo "file get content ".file_get_contents('./deg1.txt', true)."<br>";
                    $deg1_send = file_get_contents('./deg1.txt', true);
                    fclose($myfile);
       
                    echo gettype($deg).", ".gettype($deg1)."<br>";
                    echo ($deg).", ".($deg1)."<br>";
                    echo "deg1_send = ".$deg1_send."<br>";
                    echo $template->render(array('sum'=>"266.4deg",
                        'max'=>$max_percent,
                        'loss'=>$loss,
                        'color'=>$color,
                        'deg'=>$deg,
