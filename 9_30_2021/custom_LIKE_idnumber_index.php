...
if(in_array($course->id,$CFG->mofids))
    ...
    //Custom code
    $lmao = optional_param('cohortid', 0, PARAM_RAW);
    $cohortid=4553;
    $sql = "
    select * from mdl232x0_course
    where id = $courseid
";
    $get_name = $DB->get_records_sql($sql,array());
    foreach($get_name as $cohort_name){
        $filename = $cohort_name->fullname;
    }
    ...
    if($isMOF)
    {
        $CFG->mofexams=array("$courseid" => array(305,335));
    ...
    if($isMOF)
        ...
        SELECT        u.id, cm.cohortid, u.firstname + ' ' + u.lastname AS fullname,
                             (SELECT        data
                             ...
                             (SELECT        data
                               FROM            dbo.mdl232x0_user_info_data
                               WHERE        (userid = u.id) AND (fieldid = 7)) AS noicap, c.idnumber AS malop, CASE WHEN qa.sumgrades > 0 THEN qa.sumgrades ELSE 0 END AS grade1,
                         CASE WHEN
                             (SELECT        gradepass
                               FROM            mdl232x0_grade_items
                               WHERE        iteminstance = 305 AND courseid = 47) <= qa.sumgrades THEN N'Đạt' ELSE N'Không đạt' END AS grade_status1,
                         CASE WHEN qa1.sumgrades > 0 THEN qa1.sumgrades ELSE 0 END AS grade2, CASE WHEN
                             (SELECT        gradepass
                               FROM            mdl232x0_grade_items
                               WHERE        iteminstance = 335 AND courseid = 47) <= qa1.sumgrades THEN N'Đạt' ELSE N'Không đạt' END AS grade_status2
...
WHERE        (c.idnumber LIKE N'%$lmao%') order by c.idnumber, u.id
        ",array());
        foreach($listRecords as $f)
        {
            $update=GetUserProfileField($f->id);
            $row = array();
            $row[] = $f->id;
            $row[] = $f->fullname;
            $row[] = $f->ngaysinh;
            ...
            $row[] = $f->grade2;
            $row[] = $f->grade_status2;
            $row[] = $update->profile['DA'];
            $row[] = $update->profile['vanphong'];
            $export->add_data($row);
        }
